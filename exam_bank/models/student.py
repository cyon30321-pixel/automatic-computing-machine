...
def create_student(cfg, name, grade="", target="", level=""):
    ...
    with db_conn(cfg) as conn:
        cur = conn.cursor()
        cur.execute("INSERT INTO students (name, grade, target, level, created_at) VALUES (?,?,?,?,?)",
...
def update_student(cfg, sid, name=None, grade=None, target=None, level=None):
    with db_conn(cfg) as conn:
        ...
        if grade is not None: sets.append("grade=?"); params.append(grade)
        if target is not None: sets.append("target=?"); params.append(target)
        if level is not None: sets.append("level=?"); params.append(level)
        if sets: params.append(sid); cur.execute(f"UPDATE students SET {','.join(sets)} WHERE id=?", params)
...
def create_exam_record(cfg, student_id, question_ids, exam_title="", shuffle_order=None):
    ...
    with db_conn(cfg) as conn:
        cur = conn.cursor()
        cur.execute("""INSERT INTO exam_records (student_id, exam_date, exam_title, total_questions, created_at)
                       VALUES (?,?,?,?,?)""",
                    (student_id, now.strftime("%Y-%m-%d"), exam_title, len(order), now.isoformat()))
        exam_id = cur.lastrowid
        for idx, qid in enumerate(order, 1):
            cur.execute("INSERT INTO exam_items (exam_id, question_id, display_order) VALUES (?,?,?)",
                        (exam_id, qid, idx))
    ...
    with db_conn(cfg) as conn:
        ...
        items = cur.execute(
            """SELECT ei.*, q.content as q_content, q.q_num, q.answer as correct_answer,
               ...
               WHERE ei.exam_id=? ORDER BY ei.display_order""", (exam_id,)).fetchall()
...
def list_exam_records(cfg, student_id=None):
    with db_conn(cfg) as conn:
        cur = conn.cursor()
        if student_id:
            rows = cur.execute("SELECT * FROM exam_records WHERE student_id=? ORDER BY id DESC", (student_id,)).fetchall()
        else:
    ...
    with db_conn(cfg) as conn:
        ...
        items = cur.execute(
            """SELECT ei.id, ei.question_id, q.answer as correct_answer
               FROM exam_items ei JOIN questions q ON ei.question_id = q.id
               WHERE ei.exam_id=?""", (exam_id,)).fetchall()
        for ei_id, qid, correct_ans in items:
            ...
            cur.execute("UPDATE exam_items SET student_answer=?, is_correct=? WHERE id=?",
                        (student_ans, is_correct, ei_id))
        score = round((correct / total * 100), 1) if total > 0 else 0
        cur.execute("UPDATE exam_records SET is_graded=1, score=? WHERE id=?", (score, exam_id))
...
def get_question_history(cfg, question_id, student_id=None):
    with db_conn(cfg) as conn:
        ...
        params = [question_id]
        if student_id: query += " AND er.student_id=?"; params.append(student_id)
...
def get_student_question_stats(cfg, student_id, question_id):
    ...
    if not history: return {"count": 0, "correct": 0, "wrong": 0, "last_date": None}
    ...
    with db_conn(cfg) as conn:
        ...
        rows = cur.execute(
            """SELECT ei.question_id, ei.student_answer, q.answer as correct_answer,
               ...
               ORDER BY er.exam_date DESC LIMIT ?""", (student_id, limit)).fetchall()
        cols = [d[0] for d in cur.description]
        return [dict(zip(cols, r)) for r in rows]
    ...
    with db_conn(cfg) as conn:
        ...
        rows = cur.execute(
            """SELECT p.extra_tags, ei.is_correct
               ...
               JOIN questions q ON ei.question_id = q.id
               JOIN passages p ON q.passage_id = p.id
               WHERE er.student_id=? AND ei.is_correct >= 0""", (student_id,)).fetchall()
    tag_stats = {}
    for tags_str, is_correct in rows:
        tags = [t.strip() for t in (tags_str or "").split(",") if t.strip()]
        if not tags: tags = ["미분류"]
        for tag in tags:
            if tag not in tag_stats: tag_stats[tag] = {"total": 0, "wrong": 0}
            tag_stats[tag]["total"] += 1
            if is_correct == 0: tag_stats[tag]["wrong"] += 1
    result = []
    for tag, stat in tag_stats.items():
        rate = round(stat["wrong"] / stat["total"] * 100, 1) if stat["total"] > 0 else 0
        result.append({"tag": tag, "total": stat["total"], "wrong": stat["wrong"], "wrong_rate": rate})
