        ...
        for q in (questions or []):
            ...
            cur.execute(
                ...
                (pid, q.get("q_num", "-"), q.get("content", ""),
                 choices, c_type, q.get("answer", ""), q.get("explanation", ""),
                 q.get("difficulty", 3)))
    return pid
...
def update_passage(cfg, pid, content=None, answer_text=None, questions=None, **kwargs):
    ...
    with db_conn(cfg) as conn:
        ...
        if answer_text is not None: sets.append("answer_text=?"); params.append(answer_text)
        for col in ("category1","category2","school_year","exam_year","exam_month","publisher","extra_tags"):
            if col in kwargs: sets.append(f"{col}=?"); params.append(kwargs[col])
        params.append(pid)
...
def delete_passage(cfg, pid):
    with db_conn(cfg) as conn: conn.cursor().execute("DELETE FROM passages WHERE id=?", (pid,))

def delete_question(cfg, qid):
    with db_conn(cfg) as conn: conn.cursor().execute("DELETE FROM questions WHERE id=?", (qid,))
...
def get_questions_for_passage(cfg, pid):
    with db_conn(cfg) as conn:
        cur = conn.cursor()
        rows = cur.execute("SELECT * FROM questions WHERE passage_id=? ORDER BY id", (pid,)).fetchall()
...
def update_question_answer(cfg, qid, answer="", explanation=""):
    with db_conn(cfg) as conn:
        conn.cursor().execute("UPDATE questions SET answer=?, explanation=? WHERE id=?", (answer, explanation, qid))

def update_question_difficulty(cfg, qid, difficulty):
    with db_conn(cfg) as conn:
        conn.cursor().execute("UPDATE questions SET difficulty=? WHERE id=?", (difficulty, qid))
...
def search_passages(cfg, search="", category1="", category2="", exam_year=""):
    with db_conn(cfg) as conn:
        ...
        if search:
            ...
            params += [f"%{search}%"] * 3
        if category1 and category1 != "전체": query += " AND category1=?"; params.append(category1)
        if category2 and category2 != "전체": query += " AND category2=?"; params.append(category2)
        if exam_year and exam_year != "전체": query += " AND exam_year=?"; params.append(exam_year)
...
def find_duplicate(cfg, content):
    ...
    with db_conn(cfg) as conn:
        row = conn.cursor().execute("SELECT id FROM passages WHERE content_hash=?", (h,)).fetchone()
...
def auto_select_questions(cfg, count=15, category1="", category2="",
    ...
    with db_conn(cfg) as conn:
        ...
        query = """SELECT q.id, q.passage_id, q.q_num, q.content, q.difficulty, q.usage_count
                   FROM questions q JOIN passages p ON q.passage_id = p.id
                   WHERE q.difficulty BETWEEN ? AND ?"""
        params = [min_diff, max_diff]
        if category1 and category1 != "전체": query += " AND p.category1=?"; params.append(category1)
        if category2 and category2 != "전체": query += " AND p.category2=?"; params.append(category2)
        if school_year and school_year != "전체": query += " AND p.school_year=?"; params.append(school_year)
        query += " ORDER BY q.usage_count ASC, RANDOM() LIMIT ?"
        params.append(count)
        rows = cur.execute(query, params).fetchall()
        cols = [d[0] for d in cur.description]
        return [dict(zip(cols, r)) for r in rows]
