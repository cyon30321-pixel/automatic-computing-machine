...
def backup_db(cfg):
    src = get_db_path(cfg)
    if not os.path.exists(src): raise FileNotFoundError("DB 파일이 없습니다.")
...
def restore_db(cfg, backup_path):
    ...
    if backup_path.endswith(".zip"):
        with zipfile.ZipFile(backup_path, "r") as zf:
            ...
            if not db_files: raise ValueError("ZIP에 DB 파일이 없습니다.")
...
def list_backups(cfg):
    ...
    for f in sorted(os.listdir(db_dir), reverse=True):
        if ("backup" in f) and (f.endswith(".db") or f.endswith(".zip")):
            ...
            files.append({"name": f, "path": full, "size": os.path.getsize(full)})
    return files
...
def import_json(cfg, json_data):
    ...
    for item in json_data:
        ...
        create_passage(cfg, content=item.get("content", item.get("passage", "")),
            category1=item.get("category1", ""), category2=item.get("category2", ""),
            school_year=item.get("school_year", ""), exam_year=item.get("exam_year", ""),
            exam_month=item.get("exam_month", ""), publisher=item.get("publisher", ""),
            extra_tags=item.get("extra_tags", ""), answer_text=item.get("answer_text", ""),
            questions=questions)
        count += 1
    return count
