...
— v5.5: [Bug Fix] 파일명 [정답] 유출 수정, 선택지 마커 ①②③④⑤ 추가,
         정답지 지문별 중복해설 제거 (orig_num 필터링)
— v6.0: custom_title/custom_filename 지원, 파일명 sanitization,
         PermissionError 방어, docx_utils 공유 함수 활용
"""
import os
import re
import time
        ...
        for q_idx, q in enumerate(item["questions"]):
            ...
            if special_matches:
                ...
                for idx_m, sm in enumerate(special_matches):
                    block_label = sm.group(0).strip().rstrip(':').strip()
                    start = sm.end()
                    ...
                    special_blocks.append((block_label, content))
            else:
            ...
            for blk_label, content in special_blocks:
                if '<조건>' in blk_label or '[조건]' in blk_label:
                    clean_c = re.sub(r'^(<조건>|\[조건\])\s*\n?', '', content).strip()
                    create_monochrome_box(doc, "<조건>", clean_c)
                elif '[우리말]' in blk_label:
                    create_monochrome_box(doc, "우리말:", content)
                elif '[보기]' in blk_label:
                    create_monochrome_box(doc, "보기:", content)
                elif '[정답]' in blk_label or '정답' in blk_label:
...
def create_exam_files(target_dir, prefix, exam_data, cfg,
        ...
        if is_workbook:
            ...
            for q_idx_e, q in enumerate(questions_list):
                ...
                if choices and any(c for c in choices if c):
                    c_type = q.get("c_type", None)
                    if c_type is None:
                        has_alpha = any("(A)" in str(c) or "(B)" in str(c) for c in choices)
                        c_type = "alpha" if has_alpha else "num"
                    markers = (["(A)", "(B)", "(C)", "(D)", "(E)"]
                               if c_type == "alpha"
                               else ["①", "②", "③", "④", "⑤"])
                    for i, ch in enumerate(choices):
                        if ch and i < 5:
                            ch_clean = re.sub(r'^[①②③④⑤ⓐⓑⓒⓓⓔ]\s*', '', str(ch))
                            ch_clean = re.sub(r'^\([A-Ea-e]\)\s*', '', ch_clean)
                            ch_clean = re.sub(r'^\d\s*[)번]\s*', '', ch_clean).strip()
                            cp = doc_q.add_paragraph()
                            cp.paragraph_format.left_indent = Cm(1.3)
                            cp.paragraph_format.first_line_indent = Cm(-0.8)
                            cp.paragraph_format.space_before = Pt(1)
                            cp.paragraph_format.space_after = Pt(1)
                            marker_run = cp.add_run(f"{markers[i]}  ")
                            marker_run.bold = True
                            marker_run.font.size = Pt(int(font_size))
                            add_formatted_text(cp, ch_clean if ch_clean else str(ch))
    ...
    # v6.0: 사용자 지정 파일명 + sanitization
    if custom_filename:
        _safe = re.sub(r'[\\/:*?"<>|]', '_', custom_filename).strip(". ") or "untitled"
        q_stem = f"{_safe}_문제지{label}"
        a_stem = f"{_safe}_정답지{label}"
    else:
        q_stem = f"{prefix}_문제지_{ts}{label}"
        a_stem = f"{prefix}_정답지_{ts}{label}"

    q_docx = os.path.normpath(os.path.join(target_dir, q_stem + ".docx"))
    doc_q.save(q_docx)

    # answer sheet
    doc_a = docx.Document()
    _apply_doc_style(doc_a, font_name, font_size)
    ans_banner_title = f"{custom_title} — 정답 및 해설" if custom_title else f"맞춤형 {title_prefix} — 정답 및 해설"
    add_header_banner(doc_a, ans_banner_title, "", logo_path, font_name)
    ...
    for idx, item in enumerate(exam_data, 1):
        ...
        # v5.5: filter answer_text to only show lines relevant to THIS passage's questions
        ans = (item.get("answer_text") or "").strip()
        if ans:
            orig_nums_in_passage = set()
            for m_item in mappings:
                orig = m_item.get("orig_num", "")
                if orig and orig != "-":
                    try:
                        orig_nums_in_passage.add(int(orig))
                    except (ValueError, TypeError):
                        pass
            ...
            for line in ans_lines:
                ...
                if num_m:
                    if current_num is not None and (not orig_nums_in_passage or current_num in orig_nums_in_passage):
                        filtered_blocks.append((current_num, current_lines_buf))
                    current_num = int(num_m.group(1))
                    current_lines_buf = [stripped]
                else:
                    current_lines_buf.append(stripped)

            if current_num is not None and (not orig_nums_in_passage or current_num in orig_nums_in_passage):
                filtered_blocks.append((current_num, current_lines_buf))

            if filtered_blocks:
                for ans_num, block_lines in filtered_blocks:
                    new_num_display = None
                    for m_item in mappings:
                        try:
                            if int(m_item.get("orig_num", -1)) == ans_num:
                                new_num_display = m_item["new_num"]
                                break
                        except (ValueError, TypeError):
                            pass
                    ...
                    for li, line_text in enumerate(block_lines):
                        ...
                        num_m = re.match(r'^(\d+)[\.\\)번]\s*', line_text)
                        if num_m and li == 0:
                            body_p.paragraph_format.space_before = Pt(6)
                            display_num = f"{new_num_display}." if new_num_display else line_text[:num_m.end()]
                            nr = body_p.add_run(display_num + " ")
                            nr.bold = True
                            nr.font.color.rgb = RGBColor.from_string(Theme.SECONDARY)
                            body_p.add_run(line_text[num_m.end():].strip())
                        else:
                            body_p.add_run(line_text)
            else:
                no_ans = doc_a.add_paragraph()
                no_ans.add_run("(해당 문항의 해설이 없습니다.)")
    ...
    a_docx = os.path.normpath(os.path.join(target_dir, a_stem + ".docx"))
    doc_a.save(a_docx)
