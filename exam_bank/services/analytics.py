...
def chart_progress(cfg, student_id, student_name=""):
    ...
    if not history: return False, "데이터가 없습니다."
    ...
    ax.set_xlabel("평가 시기", fontsize=11); ax.set_ylabel("이해도 (100점)", fontsize=11)
    ax.set_ylim(0, 105); ax.legend(); fig.tight_layout(); plt.show()
    return True, ""
...
def chart_weakness(cfg, student_id, student_name=""):
    ...
    if not summary:
        ...
        labels = [x[0] for x in items][::-1]; values = [x[1] for x in items][::-1]
    else:
        items = [(s["tag"], s["wrong_rate"]) for s in summary if s["total"] >= 1]
        labels = [x[0] for x in items][::-1]; values = [x[1] for x in items][::-1]
    if not labels: return False, "데이터가 없습니다."
    fig, ax = plt.subplots(figsize=(9, 5))
    ...
    for b in bars:
        ...
        ax.text(b.get_width() + 0.5, b.get_y() + b.get_height() / 2, label_text, va="center", fontsize=11, fontweight="bold")
    ...
    with open(path, "w", newline="", encoding="utf-8-sig") as f:
        ...
        for r in history:
            w.writerow([r["record_date"], student_name, r["weakness_tag"],
                        f"{r['score']}점", r.get("source", "manual"), str(r["feedback"]).strip()])
    return path
...
def generate_ai_prompt(cfg, student_id):
    ...
    if student.get("grade"): lines.append(f"학년: {student['grade']}")
    if student.get("target"): lines.append(f"목표: {student['target']}")
    lines.append(f"분석 기준일: {today}")
    lines.append("")
    if weakness:
        lines.append("[누적 오답 분석]")
        for w in weakness[:5]:
            status = " ← 최약점" if w["wrong_rate"] >= 70 else (" ← 개선됨" if w["wrong_rate"] <= 30 else "")
            lines.append(f"- {w['tag']}: {w['total']}회 출제 / {w['wrong']}회 오답 (오답률 {w['wrong_rate']}%){status}")
        lines.append("")
    if wrong_answers:
        lines.append("[최근 틀린 문항 원문]")
        for i, wa in enumerate(wrong_answers[:5], 1):
            lines.append(f"{i}. ({wa['exam_date']}) {wa['q_content'][:80]}")
            lines.append(f"   → 학생 답: {wa['student_answer']} / 정답: {wa['correct_answer']}")
            if wa.get("extra_tags"): lines.append(f"   태그: {wa['extra_tags']}")
            lines.append("")
    ...
    lines.append("[요청]")
    if top_tags:
        lines.append(f"위 학생의 취약점({top_tags})을 집중 훈련할 수 있는 {level_hint}영어 문제 10문항을 만들어주세요.")
    else:
        lines.append(f"위 학생의 오답 패턴을 분석하여 {level_hint}보충 문제 10문항을 만들어주세요.")
    lines.append("")
